...
const ANDROID_NOTIFICATION_CHANNEL_ID = "kantongmas";
const ANDROID_NOTIFICATION_CHANNEL_NAME = "Kantong Mas Notifications";
const ANDROID_NOTIFICATION_SOUND = "notif";

const ANDROID_PRAYER_NOTIFICATION_CHANNEL_ID = "srawung_prayer_v1";
...
export function NotificationProvider({ children }: { children: React.ReactNode }) {
  ...
  const [settings, setSettingsState] = useState<NotificationSettings>(() => {
    const saved = localStorage.getItem("tokopembantu-notifications");
    return saved ? JSON.parse(saved) : { appointments: true, payments: true };
  });

  const setSettings = (newSettings: NotificationSettings) => {
    setSettingsState(newSettings);
    localStorage.setItem("tokopembantu-notifications", JSON.stringify(newSettings));
  };
  ...
  const channelsReadyRef = useRef<Promise<void> | null>(null);
  const ensureChannelsCreated = useCallback(async () => {
    if (!Capacitor.isNativePlatform()) return;
    if (channelsReadyRef.current) return channelsReadyRef.current;

    channelsReadyRef.current = (async () => {
      try {
    ...
    return channelsReadyRef.current;
  }, []);

  const PRAYER_SETTINGS_STORAGE_KEY = "tokopembantu:prayer_notifications";
  ...
  const requestPermission = useCallback(async () => {
    ...
    if (permission === "granted") {
      toast({
        title: "Notifikasi Aktif",
        description: "Anda akan menerima notifikasi dari Kantong Mas.",
        variant: "success",
      });
      return true;
    } else {
  ...
  }, [cancelPrayerReminders, ensureChannelsCreated, requestPermission, toast]);
