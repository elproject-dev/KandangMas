...
export function Layout({ children }: LayoutProps) {
  ...
  useEffect(() => {
    ...
    window.addEventListener("tokopembantu:app_brand_changed", sync);
    return () => {
      window.removeEventListener("storage", sync);
      window.removeEventListener("tokopembantu:app_brand_changed", sync);
    };
  }, []);
  ...
  const moreItems = useMemo(() => [
    { label: "Riwayat Transaksi", href: "/transaksi", icon: History },
    { label: "Produk", href: "/layanan", icon: ShoppingBag },
  ...
  return (
    <div className="min-h-screen bg-background text-foreground flex flex-col md:flex-row pb-16 md:pb-0">
      ...
      <header className="md:hidden sticky top-0 z-30 bg-background/80 backdrop-blur-md border-b border-border h-14 flex items-center justify-center px-4">
        <div className="font-semibold text-[32px] text-primary flex items-center gap-2" style={{ fontFamily: "'Righteous', cursive" }}>
          {appBrand}
        </div>
      </header>
      ...
      <aside className="hidden md:flex flex-col w-64 bg-sidebar border-r border-sidebar-border h-screen sticky top-0">
        <div className="h-16 flex items-center justify-center border-b border-sidebar-border">
          <div className="font-bold text-[32px] text-primary flex items-center gap-2" style={{ fontFamily: "'Righteous', cursive" }}>
            {appBrand}
          </div>
        </div>
